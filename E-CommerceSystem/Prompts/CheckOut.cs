    ...
    public partial class CheckOut : KryptonForm
    {
        ...
        private void insertToOrders(string productName, byte[] bitmap, string quantity, int total, string variant, string userID, string productID)
        {
            ...
            try
            {
                ...
                MySqlCommand insertIntoOrders = new MySqlCommand("INSERT INTO orders (productName,variant ,picture_directory, productQuantity, total, date_ordered,orderStatus, Region, Province, City, Barangay, StreetName ,userID, productID) VALUES (@productName,@variant,@picture_directory, @productQuantity, @total, @date_ordered, @orderStatus, @Region, @Province, @City, @Barangay, @StreetName , @userID, @productID)", conn);
